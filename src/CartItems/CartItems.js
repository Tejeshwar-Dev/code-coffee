...
const CartItems = () => {
    ...
    const [cartInfo, setCartInfo] = useState({ totalAmount: 0, totalQuantity: 0 })
    
    useEffect(() => {
        // set initial cart details
        let total = 0;
        let count = 0;
        ...
        setCartInfo({ totalAmount: total.toFixed(2), totalQuantity: count });
    }, [cartItems]);
        ...
        cartList = cartItems.map(cartItem => 
            <div className={`${styledClass['cart-col']} ${styledClass['cart-items']}`} key={cartItem.id}>
                ...
                <div className={styledClass['cart-price']}>
                    <p className={styledClass['item-total-price']}>{(cartItem.price * cartItem.quantity).toFixed(2)}</p>
                </div>
    ...
    return (
        <div className={styledClass['cart-modal']}>
            <header className={styledClass['header']}>
                ...
                <p className={styledClass['header-sub-title']}>{cartInfo.totalQuantity} Item(s)</p>
            </header>
            <section className={styledClass['modal-details']}>
                ...
                <div className={styledClass['cart-items-footer']}>
                    <div className={styledClass['cart-checkout-details']}>
                        <span className={styledClass['cart-checkout-title']}>Total</span>
                        <span className={styledClass['cart-total-amount']}>$ {cartInfo.totalAmount}</span>
                    </div>
